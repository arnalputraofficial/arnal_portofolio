...
| `Skills.tsx` | `/skills` | Radar/bar skill + penilaian mandiri skala 1-10 + `RegistryPanel` (verified-skill) |
...
| `AdminDashboard.tsx` | `/admin/*` | Panel page-first (FR-106). Sidebar grup **Halaman** (`PAGE_NAV` dari `PAGE_META`: global, home, career, projects, credentials, skills, about, contact, steadbyte, notfound) + grup **Sistem** (`SYSTEM_NAV`: Inbox/Sessions/Revisions/Account). Tiap halaman punya sub-tab fungsi Content/Entries/Charts (`functionsFor(pageId)` + `PAGE_SCOPE`), URL `/admin/<halaman>?fn=<content\|entries\|charts>` di-resolve oleh `resolveRoute(pathname, search)`; alias lama di-redirect via `LEGACY_PATHS`. Panel di-render scope-aware: `ContentEditor pageId hidePageSelector`, `EntriesPanel table hideTableSelector`, `ChartsPanel specIds`. Helper `entriesPathFor(table)` memetakan tabel→halaman pemilik untuk Search Console |
...
`src/admin/ChartsPanel.tsx`, `src/admin/HistoryPanel.tsx`, `src/admin/useAdminData.ts`.

### Komponen `src/components/`

**`layout/`**
- `PageIntro.tsx` -> `PageIntro({ eyebrow, title, lead?, children? })`, `StatStrip({ items, className })`.
  `items` = `{ label: string; value: React.ReactNode; hint?: string }`. **Tidak ada field `icon`.**
- `SectionHeading.tsx` -> `SectionHeading({ eyebrow?, title, description?, className?, action? })`, `PageSection({ children, className?, id? })`.
  `title` dan `description` bertipe `React.ReactNode`, bukan `string`.
- `SiteHeader.tsx` -> `SiteHeader()`, plus konstanta `navItems` (dipakai juga oleh `SiteFooter`). Tiap item hanya `{ to, label, key }`.
- **Tidak ada penomoran dekoratif apa pun di UI.** Prop `index` sudah dihapus dari `PageIntro`, `SectionHeading`, `navItems`, dan `ROUTES` di `NotFound.tsx`, bersama seluruh span angka yang menampilkannya. Jangan menghidupkannya kembali: ada dua sistem nomor yang dulu bertabrakan (nomor halaman dan nomor urut seksi) dan pemilik menilainya membingungkan. Kunci konten `*.intro.index` dan `*.section.*.index` masih ada di `src/content/` sebagai sisa dan tidak dibaca siapa pun.
...
- `SkillCharts.tsx` -> `SkillBalanceRadar({ data })`, `TopSkillsBar({ data, limit = 10 })`, `StackUsageChart()`, `ExperienceSpreadChart()`.
  ...
  `SKILL_SCALE_MAX = 10` dipakai di `SkillCharts.tsx`, `Skills.tsx`, dan `HeroScene.tsx`. Radar menggambar `level` sebagai proporsi (`level / 10 * 100`) supaya satu sumbu 0-100 tetap dipakai bersama seri evidence, sedangkan label tetap pada skala 1-10 lewat `levelOf10`. `TopSkillsBar` memakai `domain={[0, 10]}` dengan tick 1-10. Ambang warna: `>= 8` kuat, `>= 5` sedang, sisanya rendah.
...
**`three/`**
- `HeroScene.tsx` -> `HeroScene({ className, skills? })`. Canvas 3D kluster node topologi skill. Membaca data skill nyata dari `useEntries()` (atau `skills` prop) dan memetakan node ke skill. Node bersifat interaktif: dapat diputar lewat drag, dan diklik untuk menampilkan kartu inspeksi skill (nama, kategori, bar chart penguasaan 1-10, tahun mulai dikuasai atau `since`, lama pemakaian, serta tautan ke `/skills`). Dipakai di `Home.tsx`.
  - Konstanta modul: `SKILL_SCALE_MAX = 10`, `SCALE_STEPS = [1..10]`, dan helper `masteryOf(skill)` yang membulatkan lalu menjepit `skill.level` ke 1-10. `masteryOf` sengaja dipakai baik untuk tinggi bar maupun untuk angka `n/10` supaya keduanya tidak pernah berbeda; bila salah satu dihitung langsung dari `skill.level` tanpa penjepitan, nilai di luar rentang akan membuat bar dan angka tidak sinkron.
  - Kartu memakai `useSiteText()` untuk teks `Experience` (`skills.card.years`, bernilai `{count} yrs`). Label "Mastery", "In use since", dan tombol "All skills" tetap literal agar bentuk bar chart tidak bergantung pada string yang bisa disunting. Bar dibungkus satu elemen `role="img"` dengan `aria-label` berisi nilai, karena sepuluh batang kosong tidak terbaca pembaca layar.
...
**Akar `src/components/`**
- `CertificateSlideshow.tsx` -> `CertificateSlideshow()`. Tanpa prop; mengambil `certifications` dan `scansFor()` dari `useEntries()`. Merender seksi `Scans` berisi grup per sertifikat, plus viewer layar penuh (`fixed inset-0 z-[70]`, `role="dialog"`, `aria-modal`). **Mengembalikan `null` bila tidak ada scan sama sekali**, jadi halaman Credentials tidak berubah selama pemilik belum mengunggah apa pun.
...
### `src/lib/`
- `utils.ts` -> `cn(...inputs)`, `nf(value, digits = 0)` (format angka `en-US`), `monthRange(start, end)` (menghasilkan `"Mar 2021 to Now"`), `monthsBetween(start, end)`, `humanDuration(months)`, `parseStyledLines(value)` (memecah nilai per baris dan menandai baris yang diapit `*...*` sebagai bergaya), `mailtoHref(address, subject?)` (membuang spasi dan awalan `mailto:`, lalu mengembalikan URL **compose Gmail** `https://mail.google.com/mail/?view=cm&fs=1&to=...` dengan `su` bila subjek diisi; mengembalikan `""` bila tidak memuat `@` sehingga pemanggil tidak merender tautan).
...
- `types.ts` -> `ContentPageId` (10 nilai: `global`, `home`, `career`, `projects`, `credentials`, `skills`, `about`, `contact`, `steadbyte`, `notfound`), `ContentEntry = { key, page, label, default, multiline?, group?, hint? }`. `group` hanya override pengelompokan di panel admin, bukan identitas permanen.
...
- `career.ts`, `projects.ts`, `credentials.ts`, `skills.ts`, `about.ts`, `contact.ts`, `steadbyte.ts`, `notfound.ts` -> satu berkas per halaman, masing-masing mengekspor `*_CONTENT` berisi judul, lead, label statistik, judul seksi, dan deskripsinya.
  - `steadbyte.ts` menambah halaman "About Steadbyte" (rute `/about-steadbyte`) dengan entri berpola `steadbyte.<seksi>.<field>` untuk seksi: `summary`, `problem`, `role`, `honest`. `groupBySection()` di `ContentEditor` memakai `rawGroup()` + tabel `PAGE_GROUP_ORDER` + `GROUP_TITLES`, jadi judul grupnya nama manusiawi dan urutannya mengikuti alur halaman, bukan segmen kunci mentah. Kunci mati sisa penomoran dekoratif (`*.intro.index`, `*.section.*.index`) sudah dihapus dari registry.
...
- `registry.ts` -> `CONTENT_REGISTRY` (rakitan seluruh berkas halaman), `CONTENT_DEFAULTS` (`Record<string, string>`), `PAGE_META` (`{ id, title, route, description }[]`, sumber grup **Halaman** pada sidebar `AdminDashboard` dan prop `pageId` untuk `ContentEditor`), `entriesForPage(page)`, `findEntry(key)`. Di mode DEV ada pemeriksaan kunci duplikat.
...
- `chartSeries.ts` -> satu-satunya daftar grafik yang dapat diisi angkanya dari panel admin, beserta kolom tiap grafik dan angka hasil hitungannya. Tidak membaca React maupun jaringan, jadi berkas ini dipakai halaman publik dan panel sekaligus.
  - Ekspor: `ChartFieldSpec`, `ChartBarSpec`, `ChartSpec`, `ChartSources`, `CHART_SPECS`, `findChartSpec`, `chartValue(row, key)`, `chartBarCeiling(bar, rows, row?)`, `chartBarPercent(bar, row, rows)`, `roundToField(value, field)`, `computedChartRows(id, sources)`, `monthsTo(iso)`.
  - Helper privat: `clampPercent`, `unique`, `round1`. Konstanta privat: `SKILL_SCALE_MAX = 10`, `HEALTH_ROWS`.
  - Sepuluh id pada `CHART_SPECS`: `health`, `career-tenure`, `role-scope`, `credentials-validity`, `credentials-domains`, `projects-kinds`, `stack-usage`, `experience-spread`, `skill-balance`, `skill-top`.
  - `computedChartRows()` **mereplikasi aritmetika lama setiap grafik persis**, termasuk penyaring `count >= 2` dan `slice(0, 12)` pada `stack-usage`, `slice(0, 10)` pada `skill-top`, serta `left === null ? 120 : Math.max(left, -12)` pada `credentials-validity`. Ini yang membuat grafik tanpa isian manual tampil sama seperti sebelumnya.
  - `chartBarCeiling` adalah aturan satu-satunya untuk mengukur bar: plafonnya adalah nilai `max` spesifikasi **atau** baris terbesar, mana yang lebih besar, dan bila `maxField` diisi maka skala dibaca dari baris itu sendiri (itulah cara kartu Operational Status memberi tiap baris skalanya sendiri). Fungsi ini diekspor karena panel menggambar bar yang sama di samping input, dan dua salinan aturan itu akan berbeda sendiri.
  - `ChartRow = { name, values: Record<string, number> }`. Urutan baris adalah urutan gambar.
  - **Menambah grafik baru cukup menambah satu spesifikasi di sini.** Tabel dan fungsi penyimpanannya generik, jadi tidak perlu migrasi baru.
- `types.ts` -> `EntryTable`, `ENTRY_TABLE`, `StoredEntry = { id, visible, sortOrder }`, `CertificationEntry` (extends `Certification` + `StoredEntry`, `credentialUrl: string`), `CertificationImage = { id, certificationId, storagePath, caption, width, height, byteSize, sortOrder }`, `ProjectImage = { id, projectId, storagePath, caption, mimeType, width, height, byteSize, sortOrder }`, `ProjectCurvePoint = { year, budgetM, impact }`, `EntrySnapshot` (memuat `certificationImages`, `projectImages`, `projectCurve`), `ChartRow = { name, values: Record<string, number> }`, parser batas `parseSnapshot()`, `parseChartSeries()`, `isPdfScan()`, dan `publicImageUrl(storagePath)`. Helper privat: `asNumberMap`, `parseChartRow`.
...
- `EntriesProvider.tsx` -> `EntriesProvider`, `useEntries()`, dan dua tipe ringkas: `CertificationScan = { id, caption, url, storagePath }` serta `ProjectPhoto = { id, caption, url, storagePath }`.
  - `useEntries()` mengembalikan `{ all, career, projects, certifications, skills, scansFor(id), photosFor(id), curve, chartRows(id), storedCharts, photoGroups, ready, offline, isSample(table), version, reload }`.
  ...
  - `chartRows(id)` mengembalikan baris grafik tersimpan bila ada, atau `computedChartRows(id, sources)` bila tidak. `storedCharts` hanya berisi yang diisi manual, jadi **ada tidaknya kunci `id` di dalamnya adalah cara tahu sebuah grafik diisi tangan atau tidak**. Seluruh komponen grafik membaca `chartRows`, tidak ada yang menghitung sendiri.
  - Panggil `supabase` lewat `const client = supabase;` di dalam efek, bukan langsung, supaya tidak ada `null` di dalam closure.
- `useEntryWriter.ts` -> `useEntryWriter()` mengembalikan `{ busy, feedback, clearFeedback, saveCareer, saveProject, saveCertification, saveSkill, addSkillCategory, setVisible, removeEntry, reorderEntries, addImage, removeImage, reorderImages, addProjectPhoto, setProjectPhotoCaption, removeProjectPhoto, reorderProjectPhotos, saveProjectCurve, saveChartSeries }`.
  - `saveChartSeries(chart, rows)` memanggil `portfolio_save_chart_series`; larik kosong mengembalikan grafik ke hitungan entri.
  - `saveSkill` mengirim `level` (1-10) dan `since` (tahun). `addSkillCategory(name)` memanggil `portfolio_add_skill_category` dan mengembalikan `uuid` atau `null`.
...
- `AdminAuthProvider.tsx` -> `AdminAuthProvider` dan `useAdminAuth()`, mengembalikan `{ status, identity, signIn, signOut, sendPasswordReset, resetPasswordWithToken, changePassword }`.
  ...
  - **Satu browser = satu sesi admin.** `getOrCreateSessionId()` menyimpan `arnal:admin-session-id` di `localStorage`, bukan `sessionStorage`, supaya tab-tab tidak saling berebut token Supabase yang memang sudah dibagi lewat `localStorage`.
  - `loadIdentity(userId)` mengembalikan **tiga** nilai, dan pemanggilnya wajib membedakannya: `AdminIdentity` (admin), `null` (server menjawab `42501`, akun memang bukan admin, atau RPC mengembalikan nol baris), dan `undefined` (tiga percobaan baca gagal karena jaringan). Hasil disimpan di `identityCache` per `userId`.
  - `commitIdentity(identity)` adalah satu-satunya penulis `identity` + `status`; dipakai `signIn`, `resetPasswordWithToken`, dan pendengar event auth.
  - `applySeq` (ref) membuat hanya pemanggilan `applySession` terbaru yang boleh menulis status, supaya event auth yang datang terlambat tidak membatalkan login yang baru saja berhasil.
  - `endSessionLocally()` mengakhiri sesi **tanpa** memanggil server: `clearActivity()` + `setIdentity(null)` + `setStatus("signed-out")`. Baris di `portfolio_admin_sessions` sengaja tidak disentuh, karena baris yang di-revoke adalah catatan bahwa pemilik menutup perangkat itu, dan login berikutnya yang membersihkannya.
  - `applySession` **tidak** memanggil `registerSession`. Ia dijalankan pada setiap event auth, termasuk `TOKEN_REFRESHED` dan `INITIAL_SESSION` saat reload, sedangkan register menghapus `is_revoked`; memanggilnya di sana akan membuat perangkat yang sudah ditutup pemilik dari mesin lain bisa masuk lagi. Yang boleh mengklaim ulang ID sesi hanyalah `signIn`, tempat sandi baru saja diterima.
  - `registerSession()` dipanggil `signIn` saja dan di-`await` sebelum login dilaporkan sukses, supaya panel tidak pernah terbuka sebelum heartbeat punya baris untuk ditemukan.
  - Heartbeat 30 detik (`checkIdle`) hanya `touch`. `false` dari `portfolio_touch_session` langsung diperlakukan final (`signOut()`) karena server mengembalikan `false` hanya untuk baris yang di-revoke; baris yang hilang dibuat ulang oleh touch itu sendiri. Bila `isIdleExpired()` true, `signOut()` dipanggil tanpa menembak server lebih dulu.
  - **`BroadcastChannel` `arnal:admin-auth`** menyebarkan pesan `signed-out` ke tab lain pada origin yang sama. `signOut` menyiarkannya, dan sebuah efek mendengarkannya lalu menjalankan `endSessionLocally()`, karena Supabase membersihkan `localStorage` bersama tanpa pernah mengabarkan bahwa sesinya hilang.
  - Akun yang masuk tetapi tidak ada di daftar izin langsung dipaksa `signOut()`.
  - Seluruh penanganan galat baca memakai `signOut({ scope: "local" })`, bukan `signOut()`, agar tidak memicu event auth tambahan saat event sebelumnya masih diproses.
...
- `ContentEditor.tsx` -> editor per halaman. Baseline field = `drafts[key] ?? overrides[key] ?? CONTENT_DEFAULTS[key]`. Ketikan ditahan di state `local`; `dirtyKeys` bertahan saat berpindah halaman; setelah simpan berhasil, kunci dihapus dari `local`. Grup memakai `rawGroup()` + `PAGE_GROUP_ORDER` + `GROUP_TITLES` (judul manusiawi, urutan ikut halaman); grid field pendek `sm:grid-cols-2 2xl:grid-cols-3`, paragraf dan hero tetap selebar kotak; `FIELD` = textarea compact, `FIELD_INPUT` (`h-9 text-[13px]`) = input satu baris.
...
- `ChartsPanel.tsx` -> tab **Charts**: satu lipatan per grafik dari `CHART_SPECS`, berisi tabel baris yang dapat ditambah, dihapus, dan disunting, dengan pratinjau bar di bawah tiap baris.
  - Satu lipatan terbuka pada satu waktu (`open` menyimpan `id` grafik). `ChartEditor` di-`key` dengan `` `${spec.id}:${stored ? stored.length : "computed"}` `` supaya berganti grafik atau sesudah simpan memuat ulang dari angka tersimpan.
  - Tombol: `Add a row`, `Fill from the current numbers` (menyalin `chartRows(spec.id)`), `Save the chart` (mati selama tidak ada perubahan), `Clear, back to computed` (menyimpan larik kosong).
  - Pratinjau memakai `chartBarPercent(bar, row, rows)` dari `chartSeries.ts`, yaitu fungsi yang sama dengan grafik publik. Karena itu `chartBarCeiling` diekspor sebagai fungsi tersendiri: dua salinan aturan penskalaan akan berbeda sendiri.
  - Helper privat: `cloneRow`, `sameRows` (membandingkan panjang, nama per indeks, dan seluruh kunci nilai, karena **urutan adalah urutan gambar**), `ChartRowFields`, `BarPreview`.
- `HistoryPanel.tsx` -> seksi `revisions` dengan tombol `Restore`, dan seksi `activity`.
...
- `Skill` = `{ id, name, category, level, years, since, evidence[] }`.
  `level` = penilaian diri **1 sampai 10** (10 paling kuat), bukan lagi 0-100. `since` = tahun skill mulai dikuasai; `years` = lama pemakaian.
...
- `skill_level_scale_1_to_10`: perbaikan langsung di produksi (tidak ada berkas di `supabase/migrations/`), menaikkan skala `portfolio_skills.level` dari 1-5 ke **1-10**. Urutan yang dipakai wajib diikuti bila skala ini diubah lagi: `drop constraint` lebih dulu, `update ... set level = least(10, greatest(1, round(level * 2)::int))` untuk rescale nilai lama secara proporsional, baru `add constraint` 1-10. Bila `add constraint` dijalankan sebelum `update`, seluruh baris lama akan tertolak.
- `session_heartbeat_never_revives_revoked`: **keadaan akhir** sesi admin (perbaikan langsung di produksi, tidak ada berkas di `supabase/migrations/`). Aturan yang dijaga kedua fungsi ini:
  - `portfolio_touch_session(p_session_id text, p_user_agent text default null) returns boolean` = **hanya heartbeat**. Ia menyisipkan baris bila belum ada (`on conflict ... do update set last_active_at`), menyegarkan `user_agent`, dan mengembalikan `false` **tanpa menyentuh baris** bila barisnya `is_revoked = true`. Ia tidak pernah membersihkan `is_revoked`.
  - `portfolio_register_session(p_session_id text, p_user_agent text default null) returns void` = **hanya dipanggil setelah sandi diterima**. Ia boleh `set is_revoked = false, revoked_at = null, created_at = now()`, sehingga browser yang ID sesinya sudah dicabut tidak terkunci permanen.
  - Keduanya `security definer` dengan `set search_path to 'public', 'pg_temp'`, dan keduanya wajib `revoke execute ... from anon, public` + `grant execute ... to authenticated`; tanpa itu `anon` ikut bisa memanggilnya karena hak bawaan Supabase.
  - Kontrak yang diuji dan harus tetap benar: register -> touch `true` -> revoke -> touch `false` -> register -> touch `true`.
  - Migrasi lain yang menyertainya: `revive_heartbeat_and_share_session_across_tabs` (percobaan pertama, **salah** karena touch ikut menghidupkan baris yang di-revoke; keputusan ini dibatalkan) dan `touch_session_revoke_public_execute` (penutupan hak `anon`).
- `20260921000000_portfolio_chart_series.sql`: angka grafik yang dapat diisi manual dari panel admin. Tabel `portfolio_chart_series` (satu baris per grafik, `chart` sebagai kunci utama) dan fungsi `portfolio_save_chart_series(p_chart text, p_rows jsonb)`. Fungsi itu memangkas nama baris ke 120 karakter, membatasi kunci kolom pada `^[a-zA-Z0-9_]{1,32}$`, mencocokkan nilai sebagai **teks** dulu (`^-?[0-9]{1,9}(\.[0-9]{1,6})?$`) sebelum di-cast supaya nilai seperti `true` atau `nan` ditolak, menjepit nilai ke `[-1000000, 1000000]`, menolak larik di atas 40 baris dan muatan di atas 20 KB, menghapus baris saat lariknya kosong, dan menulis `portfolio_activity`. Tabelnya generik, jadi **tidak perlu migrasi baru untuk menambah grafik**.
- `harden_anon_grants_and_event_trigger_execute`: **pengerasan hak tabel** (perbaikan langsung di produksi, tidak ada berkas di `supabase/migrations/`). Bootstrap Supabase pernah memberi hak `all` pada setiap tabel `public` kepada `anon` dan `authenticated`, dan hanya `INSERT`/`UPDATE`/`DELETE` yang pernah dicabut, sehingga `TRUNCATE`, `REFERENCES`, dan `TRIGGER` tertinggal. Migrasi ini mencabut ketiganya dari seluruh tabel `public`, menambahkan `alter default privileges ... revoke` supaya tabel baru tidak mewarisinya, dan mencabut `execute` pada `rls_auto_enable()` dari `anon`, `authenticated`, dan `public`.
- `revoke_leftover_maintain_privilege`: mencabut sisa hak `MAINTAIN` (PostgreSQL 17: `VACUUM`, `ANALYZE`, `REINDEX`, `CLUSTER`, `LOCK TABLE`) dari `anon` dan `authenticated` pada seluruh tabel `public`, plus `alter default privileges` yang sama. Ditemukan tepat setelah migrasi di atas, karena `MAINTAIN` tidak termasuk dalam daftar yang dicabut pertama kali.
- Keadaan akhir hak yang harus dijaga: `anon` hanya memegang `SELECT` pada tabel yang memang publik (`portfolio_career`, `portfolio_certifications`, `portfolio_certification_images`, `portfolio_content`, `portfolio_data_version`, `portfolio_entity_usage`, `portfolio_media`, `portfolio_project_curve`, `portfolio_project_images`, `portfolio_projects`, `portfolio_skill_category_options`, `portfolio_skills`) dan **tidak memegang hak apa pun** pada `portfolio_activity`, `portfolio_admin_sessions`, `portfolio_admins`, `portfolio_drafts`, `portfolio_messages`, `portfolio_revisions`. `authenticated` memegang `SELECT` pada seluruh tabel, ditambah `INSERT`/`UPDATE`/`DELETE` hanya pada `portfolio_content`, `portfolio_drafts`, `portfolio_media`, dan `DELETE`/`UPDATE` pada `portfolio_messages`. Periksa ulang dengan `has_table_privilege`, jangan dengan membaca berkas migrasi.
...
- Validasi: nama 2 sampai 100 (`MAX_NAME`), surel 5 sampai 200 (`MAX_EMAIL`) dengan pola,
  topik maksimal 120 (`MAX_TOPIC`, sejalan dengan pemotongan di `portfolio_send_message`),
  pesan 20 sampai 5000 karakter. `topic` kosong diganti `DEFAULT_TOPIC` (`"General inquiry"`).
- **Karakter kendali ditolak, bukan dipotong.** `CONTROL_CHARS` (`/[\u0000-\u001f\u007f]/`, sengaja
  tanpa flag `g` supaya `lastIndex` tidak menipu) diuji pada `name` dan `topic`. Keduanya masuk ke
  subjek surel, jadi baris baru di sana berpotensi menyusupkan header surel tambahan. Nilai yang
  panjangnya di luar batas **ditolak `400`**, tidak dipotong diam-diam supaya pengunjung tidak
  mengira pesannya terkirim utuh.
- Kalau endpoint ini diuji langsung, uji lewat bundel sementara di luar repo
  (`npx esbuild api/contact.ts --bundle --platform=node --format=cjs --outfile="$env:TEMP\...cjs"`),
  dan pastikan setiap payload yang dipakai memang **gagal validasi**, karena payload yang lolos akan
  benar-benar menulis baris ke `portfolio_messages`. Hapus baris percobaan itu setelah selesai.
...
| `portfolio_chart_series` | Angka grafik manual dari tab Charts: `chart` (PK, pola `^[a-z0-9-]{1,40}$`), `rows` (`jsonb`, larik baris `{ name, values }`), `updated_at`, `updated_by`. Tidak ada baris berarti grafik memakai hitungan entri | RLS aktif, hanya `SELECT` untuk `anon` dan `authenticated`. Tulis hanya lewat `portfolio_save_chart_series` |
...
- `portfolio_skills`: `name` 1-160, `level` **1-10**, `years` 0-60, `since` 1980-2100.
...
7. Bila tugas menambah **halaman publik baru**, sentuh tujuh titik ini berurutan: (a) `src/content/<page>.ts` baru, (b) anggota baru pada union `ContentPageId` di `src/content/types.ts`, (c) impor + spread + entri `PAGE_META` di `src/content/registry.ts`, (d) kunci `nav.<page>` di `src/content/global.ts`, (e) berkas halaman di `src/pages/`, (f) `<Route>` di `src/App.tsx`, (g) entri `navItems` di `src/components/layout/SiteHeader.tsx` (footer ikut otomatis). Tambahkan juga rute ke `ROUTES` di `NotFound.tsx` beserta pasangan `notfound.route.N.label`/`.hint`. Tidak ada nomor urut yang perlu disesuaikan; halaman baru cukup ditambahkan dengan `{ to, label, key }`.
...
- **RLS tidak memfilter `TRUNCATE`.** Row level security hanya berlaku untuk `SELECT`, `INSERT`, `UPDATE`, dan `DELETE`. Satu pernyataan `TRUNCATE` dapat mengosongkan tabel ber-RLS tanpa memandang policy apa pun. Karena itu hak `TRUNCATE` harus dilihat sebagai bagian dari permukaan serangan, bukan sekadar hak administratif. Bootstrap Supabase pernah meninggalkannya (bersama `REFERENCES` dan `TRIGGER`) pada `anon` dan `authenticated` di seluruh tabel `public`. Jangan berhenti di `INSERT`/`UPDATE`/`DELETE` saat memeriksa `relacl`; periksa juga `MAINTAIN`, dan jangan lupa `alter default privileges` supaya tabel baru tidak mewarisinya.
- **Alias `cmd` tidak boleh dipakai pada `pg_policy` di kueri MCP.** `cmd` adalah kata kunci di jalur ini dan kueri gagal dengan `the cmd does not exist`. Pakai alias lain, misalnya `action`.
- **`pg_policy.polqual` tampil sebagai pohon internal**, bukan SQL yang bisa dibaca. Untuk memastikan sebuah policy memakai pemeriksaan admin, jangan membaca `polqual`; lihat saja policy yang sama lewat `pg_policies.qual`, atau periksa definsi fungsi yang dipanggilnya.
- **`aclexplode` mengembalikan `grantee` berupa OID, bisa `0` untuk `PUBLIC`.** Bila difilter `rolname in ('anon','authenticated')` tanpa memperhatikan `0`, hasilnya tampak bersih padahal `PUBLIC` masih memegang hak. Periksa juga baris `grantee = 0`.
...
- **`mailtoHref()` tidak menghasilkan `mailto:` lagi, melainkan URL compose Gmail.** Kliknya membuka tab baru, jadi setiap pemanggilnya **wajib** diberi `target="_blank"` dan `rel="noreferrer noopener"`. Tanpa itu pengunjung meninggalkan portofolio. Konsekuensi lain: pemeriksaan `href.startsWith("http")` untuk menentukan `target` kini bernilai benar pada tautan surel, jadi jalur itu ikut terpakai. Bila menambah pemakai baru, jangan menulis `mailto:` mentah dan jangan mengandalkan `encodeURIComponent` sendiri; pakai `mailtoHref` supaya pengodean dan penolakan nilai tanpa `@` seragam. Penanda entri sosial "Email" di `src/data/portfolio.ts` sengaja disimpan sebagai `"mailto:"` tanpa alamat, jadi `startsWith("mailto:")` tetap cara yang benar untuk mengenali kanal itu.
...
- **`supabase.rpc(...)` dan `supabase.from(...)` tidak mengirim permintaan bila tidak di-`await`.** `PostgrestBuilder` di `@supabase/postgrest-js` 2.116.0 menjalankan `fetch` di dalam `then()`, jadi `void supabase.rpc(...)` membuat builder lalu membuangnya tanpa pernah mengirim apa pun.
- **Race condition pembacaan allowlist (`loadIdentity`) saat event auth pertama.** Saat `signInWithPassword` selesai, token JWT di client SDK memerlukan beberapa milidetik untuk terpasang penuh pada request builder. Pemanggilan RPC allowlist harus menyertakan mekanisme coba-ulang (retry) singkat untuk kegagalan transien sebelum menyimpulkan pengguna bukan admin (jangan langsung memicu `signOut()`).
- **`portfolio_drafts` hanya boleh di-query jika ada session aktif.** Melakukan query `portfolio_drafts` untuk publik visitor akan memicu HTTP 401 merah di konsol peramban karena dilindungi RLS autentikasi admin.
- **`portfolio_touch_session` mengembalikan `false` hanya untuk baris yang benar-benar di-revoke.** Baris yang tidak ada justru **dibuat ulang** oleh touch (`on conflict ... do update`), jadi `false` boleh diperlakukan final. Aturan yang berlaku sekarang: touch tidak pernah membersihkan `is_revoked`, dan yang boleh mengklaim ulang ID sesi hanyalah `portfolio_register_session` yang dipanggil **setelah sandi diterima**. Sebelumnya sempat dibalik (touch ikut menghidupkan baris yang di-revoke), dan akibatnya tombol "Logout session" di panel tidak berfungsi. Untuk menguji kontrak ini, jalankan berurutan: register -> touch `true` -> revoke -> touch `false` -> register -> touch `true`.
- **Gejala "habis enter malah disuruh isi sandi lagi" bukan bug sandi.** Sebelum menyalahkan kata sandi, baca `auth_logs` dan `auth_audit_logs` lewat `query_logs`: bila ada `POST /token?grant_type=password` 200 dan `login` untuk akun itu, sandinya benar dan masalahnya ada di lapisan sesi klien. Penyebab yang pernah terbukti: ID sesi admin disimpan di `sessionStorage` (terpisah per tab) padahal token Supabase berada di `localStorage` (dibagi antar tab), sehingga beberapa tab saling menembak heartbeat ke akun yang sama dan memicu `logout?scope=global`.
- **`loadIdentity` mengembalikan `null` dan `undefined` untuk hal yang berbeda, dan menyamakannya adalah bug yang mengeluarkan pemilik saat jaringan bermasalah.** `null` = server menjawab `42501` atau nol baris (memang bukan admin); `undefined` = tiga percobaan baca gagal (gangguan jaringan). Jangan pernah menulis `if (!next)`; tulis `next === null` untuk keluar dan `next === undefined` untuk menahan status apa adanya. Pola ini wajib dipakai di setiap tempat yang memanggil `loadIdentity`.
- **`supabase.auth.signOut()` tanpa argumen di dalam penanganan event auth memicu event auth tambahan**, dan itu bisa saling menimpa dengan event yang sedang diproses. Untuk kasus "sesi sudah hilang, sudahi saja", pakai `signOut({ scope: "local" })` yang hanya membersihkan `localStorage`.
- **Satu browser berbagi satu sesi admin.** `localStorage` dibagi antar tab, jadi `BroadcastChannel` `arnal:admin-auth` dipakai untuk menyiarkan `signed-out`: tanpa itu tab yang sedang di latar tidak tahu sesinya sudah hilang sampai heartbeat berikutnya.
- **`registerSession` tidak boleh dipanggil dari `applySession`.** `applySession` berjalan pada setiap event auth termasuk `TOKEN_REFRESHED`, sedangkan register menghapus `is_revoked`; menaruhnya di sana membuat perangkat yang sudah ditutup pemilik dari mesin lain bisa masuk lagi lewat refresh token.
