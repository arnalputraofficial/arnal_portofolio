...
import { parseStyledLines, mailtoHref } from "@/lib/utils";
...
export function SiteFooter() {
  ...
  const closingLines = parseStyledLines(t("footer.closing"));
        ...
        <div className="md:col-span-5">
          <p className="font-display text-2xl font-semibold leading-snug tracking-tight text-ink-50 text-balance sm:text-3xl">
            {closingLines.map((line, i) => (
              <span
                key={`${line.text}-${i}`}
                className={line.styled ? "block text-ink-400" : "block"}
              >
                {line.text}
              </span>
            ))}
        ...
        <div className="md:col-span-4">
          ...
          <ul className="mt-5 space-y-2.5">
            {profile.socials.map((s) => {
              // The email entry points at the editable address instead of the
              // frozen value in the data file, so both stay in sync.
              const href = s.href.startsWith("mailto:")
