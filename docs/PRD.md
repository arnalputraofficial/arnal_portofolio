...
- **Perbaikan Gagal Login Admin yang Terlempar Kembali ke Halaman Login**: setelah kata sandi diterima, panel admin sempat tidak mau terbuka; pemilik melaporkan, "Ada error tidak bisa login. Tadi sempat bisa tapi malah masuk ke login page lagi."
  ...
  - Keadaan yang salah: sesi admin dianggap dicabut dari perangkat lain beberapa detik setelah login, sehingga pengguna ditendang keluar berulang kali tanpa ada yang mencabutnya.
  - Penyebab yang ditemukan: tiga pemanggilan `supabase.rpc(...)` ditulis sebagai `void supabase?.rpc(...)`. Pustaka `postgrest-js` yang dipakai proyek ini **baru mengirim permintaan ketika builder-nya di-`await`**, karena di dalam `PostgrestBuilder.ts` pemanggilan `fetch` berada di dalam `then()`. Builder yang dibuat lalu dibuang tanpa `await` tidak mengirim apa pun. Akibatnya `portfolio_register_session` tidak pernah sampai ke basis data, baris di `portfolio_admin_sessions` tidak pernah ada, dan `portfolio_touch_session` selalu mengembalikan `false` karena barisnya tidak ditemukan. Detak jantung panel menafsirkan `false` sebagai "dicabut oleh sesi admin lain", lalu memanggil keluar. Karena baris sesi tidak pernah dibuat, gejala ini berulang tanpa henti.
  - Bukti: log Auth Supabase menunjukkan `/token` berstatus 200 pada pukul 07:39:18, 07:39:44, dan 07:39:51, jadi kredensialnya benar. Log Edge menunjukkan `portfolio_admin_state`, `portfolio_touch_session`, dan `portfolio_revoke_session` terpanggil dari peramban pemilik, tetapi `portfolio_register_session` **tidak pernah sekali pun** muncul. `portfolio_admin_sessions` berisi nol baris. Galat 400 pada tangkapan layar pemilik adalah percobaan kata sandi yang salah pada pukul 07:39:48, dan galat 403 adalah `/auth/v1/logout` bercorak `Session not found` akibat keluar dua kali, jadi keduanya gejala, bukan sebab.
  - Pemeriksaan yang menggugurkan tersangka lain: `portfolio_admins` berisi satu baris yang cocok, `portfolio_admin_state` menjawab 200, hak `EXECUTE` untuk `authenticated` ada pada keenam fungsi sesi, badan keenam fungsi benar, dan uji transaksi membuktikan `portfolio_register_session` mampu menyisipkan baris saat dipanggil sebagai `authenticated` dengan klaim surel yang benar.
  - Perubahan kode pada `src/admin/AdminAuthProvider.tsx`: ditambahkan `registerSession()` sebagai satu satunya jalan mendaftarkan sesi, dan isinya memakai `await` serta menulis peringatan ke konsol bila gagal, sehingga kegagalan tidak lagi senyap. Pemanggilan lama di `applySession` dan `signIn` diganti dengan `await registerSession()`. Detak aktivitas `handleActivity` kini memanggil `pingSession()` yang juga meng-`await` di dalam. Pemeriksaan pencabutan `checkIdle` tidak lagi menendang sesi pada `false` pertama: ia mendaftarkan ulang lebih dulu, lalu memanggil `portfolio_touch_session` sekali lagi, dan hanya keluar bila jawaban kedua masih `false`. Cara ini aman karena `portfolio_register_session` memakai `on conflict ... do update` yang tidak pernah menghapus tanda `is_revoked`, jadi sesi yang benar benar dicabut tetap dilaporkan `false`.
  - Tambahan kecil: `applySession` memeriksa ulang penjaga `seq` setelah pendaftaran, agar jawaban yang sudah basi tidak menulis status.
  - Verifikasi: `npx tsc --noEmit` keluar 0 dan `npm run build` berhasil (`built in 12.42s`). Mekanisme pustaka dibuktikan dengan probe `@supabase/postgrest-js` 2.116.0 yang menghitung pemanggilan `fetch`: dua builder, satu di-`await` dan satu dibuang, menghasilkan tepat satu panggilan, sehingga pola lama memang tidak mengirim apa pun. Rantai basis data diuji di dalam transaksi yang di-`rollback`: `portfolio_register_session` lalu `portfolio_touch_session` mengembalikan `true`, dan `register` lalu `revoke` lalu `touch` mengembalikan `false`.
  - Catatan jujur: perbaikan belum diuji dengan login sungguhan dari peramban setelah perubahan ini. Yang sudah pasti adalah mekanismenya, bukan hasil akhirnya di layar pemilik.
  - Status: Selesai untuk sisi kode. Perlu satu kali login ulang oleh pemilik untuk memastikan panel bertahan.
